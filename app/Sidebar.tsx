...
export default function Sidebar() {
  ...
  const getFillColor = (href) => {
    if (isActive(href) || hoverLink == href){
      return "#1141CB"
    }
    return "#111111"
  }
  ...
  const blog = (fill) => <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20" fill="none">
      ...
      <div className='w-[280px]  bg-[#FAFBFC]  justify-between h-screen border-1 border-gray-200   md:flex md:flex-col flex-1/2'>
        <div className=''>
          ...
          <div className='flex flex-col space-y-1'>
            <Link href={"/dashboard"} onMouseEnter={() => setHoverLink("/dashboard")} onMouseLeave={() => setHoverLink(null)} className={linkClasses("/dashboard")}>
              <div>{dashboard(getFillColor("/dashboard"))}</div>

              <span  >Dashboard</span>
            </Link>
            <Link href={"/manage"} onMouseEnter={() => setHoverLink("/manage")} onMouseLeave={() => setHoverLink(null)} className={linkClasses("/manage")}>
              <div>{manage(getFillColor("/manage"))}</div>

              <span  >Manage Bookings</span>
            </Link>
            <Link href={"/services"}  onMouseEnter={() => setHoverLink("/services")} onMouseLeave={() => setHoverLink(null)} className={linkClasses("/services")}>
            <div>{services(getFillColor("/services"))}</div>
              <span >Services</span>
            </Link>
            <Link href={"/schedule"} onMouseEnter={() => setHoverLink("/schedule")} onMouseLeave={() => setHoverLink(null)}  className={linkClasses("/schedule")}>
              <div>{schedule(getFillColor("/schedule"))}</div>
              <span  >Schedule Calendar</span>
            </Link>
            <Link href={"/blog"}  onMouseEnter={() => setHoverLink("/blog")} onMouseLeave={() => setHoverLink(null)} className={linkClasses("/blog")}>
            <div>{blog(getFillColor("/blog"))}</div>
              <span >Blog</span>
            </Link>
